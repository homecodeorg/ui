...
class Popup extends Component<T.Props, T.State> {
  ...
  static getDerivedStateFromProps({ disabled }: T.Props) {
    if (disabled) return { open: false }; // close when receive disabled=true
    return null;
  }
  ...
  open = () => {
    ...
    this.setState({ isOpen: true, isContentVisible: true }, () => {
  ...
  renderTrigger() {
    const { trigger, content, disabled, controllable } = this.props;
